...
public class Network {
    ...
    public void setNumMirrors(int newMirrors, int simTime) {
        ...
        if (newMirrors > mirrors.size()) { // create new mirrors
            if (this.strategy instanceof NConnectedTopology) {
                strategy.modifyNetworkAfterAddMirror(this, newMirrors - mirrors.size(), props, simTime);
            } else {
    ...
    private int getNumHops() {
        ...
        if (getTopologyStrategy() instanceof BalancedTreeTopologyStrategy)
            return (int) (Math.round(Math.log((getNumTargetMirrors() + 1) / 2f) / Math.log(getNumTargetLinksPerMirror())));
